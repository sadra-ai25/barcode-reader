## save unique barcodes in db (without duplicating)
...
def save_image_to_folder(cropped_image, barcode):
    ...
    timestamp = datetime.now(ZoneInfo("Asia/Tehran")).strftime('%Y%m%d_%H%M%S')
    image_filename = os.path.join(CROPPED_IMAGES_PATH, f"{timestamp}.jpg")
...
def check_barcode_exists(conn, barcode):
    try:
        cursor = conn.cursor()
        cursor.execute('''
            SELECT COUNT(*) FROM frame_barcodes WHERE barcode = ?
        ''', (barcode,))
        ...
        try:
            ...
            def callback(ch, method, properties, body):
                try:
                    ...
                    dt = datetime.fromtimestamp(timestamp, tz=ZoneInfo("Asia/Tehran"))  # ← With timezone
                    ...
                    if barcode:
                        ...
                        if not check_barcode_exists(conn, barcode):
                            ...
                            logger.info(f"Barcode {barcode} already exists, skipping save.")
    ...
    try:
        while camera_running:
            ...
            try:
                process = (
                    ffmpeg
                    .input(source, rtsp_transport='tcp', skip_frame='nokey')
                    .output('pipe:', format='rawvideo', pix_fmt='bgr24', vcodec='rawvideo', r=2, vsync='0')
        ...
        if connection and not connection.is_closed:
            connection.close()
        logger.info(f"Stream for {source_id} stopped, camera_running={camera_running}")
